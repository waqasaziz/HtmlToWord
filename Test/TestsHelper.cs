namespace Tests
{
    using Domain.Data;
    using Domain.Security;
    using Microsoft.EntityFrameworkCore;
    using System;
    using System.Text;

    internal class InMemoryEncryptionKeyProvider : IEncryptionKeyProvider
    {
        public string PrivateKey => "";
        public string PublicKey => "";

    }

    internal class TestsHelper
    {
        private HtmlToWordDbContext _context;

        internal static SHA256HashingProvider HasingProvider { get; } = new SHA256HashingProvider();

        internal static RSAEncryptionProvider EncryptionProvider { get; } = new RSAEncryptionProvider(RSAType.RSA2, Encoding.UTF8, new InMemoryEncryptionKeyProvider());

        public static HtmlToWordDbContext CreateInMemoryDbContext() => new TestsHelper().CreateDbContext();

        private HtmlToWordDbContext CreateDbContext()
        {
            var options = new DbContextOptionsBuilder<HtmlToWordDbContext>()
                       .UseInMemoryDatabase($"MovieStore-{ Guid.NewGuid().ToString()}")
                       .Options;

            _context = new HtmlToWordDbContext(options);

            return _context;
        }
    }
}
